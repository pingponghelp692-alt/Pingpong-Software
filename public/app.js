...
function applySellerOfficialTags(sellerEl, officialEl, activeBadges) {
...
function connectSocket() {
  ...
  socket.on("seat-update", (data) => {
    if (data.action === "take") {
      ...
      seatMap[data.seatNumber] = { userId: data.userId, socketId: data.socketId, userName: data.userName, userPhoto: data.userPhoto, role: data.role, activeFrame: data.activeFrame || null, vipLevel: data.vipLevel || 0, level: Number(data.level) || 1, customTag: data.customTag || null, nameEffect: data.nameEffect || null };
  ...
  socket.on("user_badges_update", (data) => {
    ...
    if (data.userId === me.userId) {
      ...
      applySellerOfficialTags($("profile-seller-tag"), $("profile-official-tag"), me.activeBadges);
      applySellerOfficialTags($("sheet-seller-tag"), $("sheet-official-tag"), me.activeBadges);
    }
    if (otherProfileUser && otherProfileUser.userId === data.userId) {
      ...
      applySellerOfficialTags($("other-seller-tag"), $("other-official-tag"), otherProfileUser.activeBadges);
    }
  });
  ...
  socket.on("badge_catalog_update", (data) => {
    ...
    applySellerOfficialTags($("profile-seller-tag"), $("profile-official-tag"), me && me.activeBadges);
    if (currentRoom) renderSeats(seatsFromMap());
  });
...
function renderSeats(seats) {
  ...
  seats.forEach((seat, i) => {
    ...
    // BUGFIX: inline style.color/textShadow always wins over the
    // .name-fx-* CSS classes (inline style beats any class selector,
    // and for gradient effects it also stomps the `color:transparent`
    // + background-clip:text trick those classes rely on) — so any seat
    // that had BOTH a VIP nameEffect AND a customTag.color used to
    // render as a frozen, non-animated solid color instead of the VIP
    // animation. The VIP name effect (paid/admin-assigned) now always
    // takes priority; the custom tag color only applies when there is
    // no active nameEffect.
    if (seat && !seat.nameEffect && seat.customTag && seat.customTag.color) {
...
async function openSeatProfileSheet(userId, seatNumber) {
  ...
  if (isMe) {
    ...
    applySellerOfficialTags($("sheet-seller-tag"), $("sheet-official-tag"), u.activeBadges);
...
function hideCallOverlay() {
  ...
  $("call-video-local").classList.add("hidden");
}
...
function renderRechargePackages() {
  ...
  packages.forEach((p) => {
    ...
    card.innerHTML = `
      ...
      <div class="coins"><img src="/images/icons/icon-diamond.png" class="currency-icon" alt="diamond"> ${p.totalCoins.toLocaleString()}</div>
  ...
  list.forEach((t) => {
    ...
    row.innerHTML = `
      <div class="user-row-body">
        <span class="name">₹${t.priceINR} → <img src="/images/icons/icon-diamond.png" class="currency-icon" alt="diamond"> ${t.totalCoins.toLocaleString()} <span class="recharge-status-badge recharge-status-${t.status}">${t.status}</span></span>
...
async function loadCoinCenterOpLog() {
  ...
  r.log.forEach((entry) => {
    ...
    const row = document.createElement("div");
    row.className = "user-row";
    row.innerHTML = `<div class="user-row-body"><span class="name">${escapeHtml(entry.targetName || entry.targetUserId)}</span><span class="sub"><img src="/images/icons/icon-diamond.png" class="currency-icon" alt="diamond"> ${entry.amount} · ${escapeHtml(entry.reason || "No note")} · ${new Date(entry.time).toLocaleString()}</span></div>`;
    wrap.appendChild(row);
  });
}
...
function renderRankingPreview() {
  ...
  if (!top3.length) { wrap.innerHTML = "🏆"; wrap.classList.add("empty"); return; }
...
function chHideOverlay() {
  ...
  $("hostcall-video-local").classList.add("hidden");
...
async function chAcquireLocalMedia(type) {
  ...
  if (type === "video") {
    // BUGFIX 2026-08-22: same fix as the private-call flow above — only
    // show the local self-preview here; the full remote wrap now paints
    // above the avatar/name/status panel, so it stays hidden until the
    // host's video track actually arrives in ontrack().
