...
function initOtpService({ DATA_FOLDER, safeRead, safeWrite } = {}) {
    ...
    const RESEND_COOLDOWN_MS = envInt("OTP_RESEND_COOLDOWN_SECONDS", 300) * 1000;
    ...
    function issueOtp(mobile) {
        ...
        if (existing && existing.expiresAt > Date.now()) {
            return { error: { code: "resend-cooldown", retryAfterSec: Math.max(1, Math.ceil((existing.expiresAt - Date.now()) / 1000)) } };
        }
    ...
    function verifyOtp(mobile, submittedOtp) {
        ...
        const normalizedOtp = String(submittedOtp || "").trim().replace(/[০-৯]/g, d => String("০১২৩৪৫৬৭৮৯".indexOf(d))); const candidateHash = hashOtp(normalizedOtp, rec.salt);
