...
function performTakeSeat(room, { userId, socketId, seatNumber }) {
    ...
    room.seats[seatNumber - 1] = { userId: found.user.userId, socketId, userName: found.user.name, userPhoto: found.user.photo || "", activeFrame: found.user.activeFrame || null, vipLevel: found.user.vipLevel || 0, level: found.user.level || found.user.idLevel || 1, customTag: found.user.customTag || null, nameEffect: found.user.nameEffect || null };
    io.to(room.roomId).emit("seat-update", {
...
io.on("connection", (socket) => {
    ...
    socket.on("mod-move-seat", ({ roomId, targetUserId, seatNumber }) => {
        ...
        io.to(roomId).emit("seat-update", { action: "move", fromSeatNumber: fromIdx + 1, seatNumber: destIdx + 1, userId: targetUserId, socketId: seatData.socketId, userName: seatData.userName, userPhoto: seatData.userPhoto, activeFrame: seatData.activeFrame || null, vipLevel: seatData.vipLevel || 0, level: seatData.level || 1, customTag: seatData.customTag || null, nameEffect: seatData.nameEffect || null, role: roleForUser(room, targetUserId) });
        io.to(roomId).emit("room-state", publicRoom(room));
        ...
        const reject = (reason) => {
            if (socket.userId) {
                ...
                socket.emit("fruitwheel-bet-rejected", {
                    ...
                    balance: found2 && found2.user ? Number(found2.user.coins) || 0 : 0
                });
            } else {
